...
// Texture Coords wrpper
BufferObject TextureCoords;
...
void sphere(float sphereRadius, int sectorCount, int stackCount, std::vector<glm::vec3>& vertex, std::vector<glm::vec3>& normal, std::vector<glm::ivec3>& tria) {
    ...
    for (int i = 0; i <= stackCount; ++i) {
        ...
        for (int j = 0; j <= sectorCount; ++j) {
            ...
            glm::vec3 normalVal = sphereVertexPos / sphereRadius;
...
int main(void)
{
    ...
    // initilize textrue coord array buffer
    TextureCoords.init();
    ...
    texcoords.resize(VN.size());
    for(int i = 0; i < VN.size(); i++){
        texcoords[i] = (glm::vec2(atan2(VN[i].x, VN[i].z) / (2. * M_PI) + 0.5, -asin(VN[i].y) / M_PI + 0.5 ));
    }
    TextureCoords.update(texcoords);
    ...
    std::ifstream fragShaderFile("../shader/task1/fragment.glsl");
    std::stringstream fragCode;
    fragCode << fragShaderFile.rdbuf();
    // load vertex shader file
    std::ifstream vertShaderFile("../shader/task1/vertex.glsl");
    ...
    program.bindVertexAttribArray("tex", TextureCoords);
    ...
    // load fragment shader file 
    std::ifstream fragShaderFileFrame("../shader/task2/fragment.glsl");
    std::stringstream fragCodeFrame;
    fragCodeFrame << fragShaderFileFrame.rdbuf();
    // load vertex shader file
    std::ifstream vertShaderFileFrame("../shader/task2/vertex.glsl");
    std::stringstream vertCodeFrame;
    vertCodeFrame << vertShaderFileFrame.rdbuf();

    quad_program.init(vertCodeFrame.str(), fragCodeFrame.str(), "newColor");
    quad_program.bind();
